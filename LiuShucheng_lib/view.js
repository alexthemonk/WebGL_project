...
function getFreeLookAt() {
  ...
  return [
    g_freeCamPos[0] + Math.cos(g_freeTheta) / len,
    g_freeCamPos[1] + Math.sin(g_freeTheta) / len,
...
function getThirdCamPos() {
  ...
  return [
    g_thirdLookAt[0] - (2 * Math.cos(g_thirdTheta)) / len,
    g_thirdLookAt[1] - (2 * Math.sin(g_thirdTheta)) / len,
...
function mouseMove(ev, callback = NaN) {
  ...
  if (callback) callback(x - g_xMclik, y - g_yMclik);

  g_xMclik = x;
  g_yMclik = y;
}

function mouseUp(ev, callback = NaN) {
  // Create right-handed 'pixel' coords with origin at WebGL g_canvas LOWER left;
  var rect = ev.target.getBoundingClientRect(); // get g_canvas corners in pixels
  var xp = ev.clientX - rect.left; // x==0 at g_canvas left edge
  var yp = g_canvas.height - (ev.clientY - rect.top); // y==0 at g_canvas bottom edge
...
function moveView(direction) {
  switch (direction) {
    case 1:
      ...
      if (g_freeCam) {
        g_freeCamPos = [
          g_freeCamPos[0] + g_cameraMoveRate * Math.cos(g_freeTheta),
          g_freeCamPos[1] + g_cameraMoveRate * Math.sin(g_freeTheta),
          g_freeCamPos[2] + g_cameraMoveRate * Math.sin(g_freePhi)
        ];
      } else {
        g_thirdLookAt = [
          g_thirdLookAt[0] + g_cameraMoveRate * Math.cos(g_thirdTheta),
          g_thirdLookAt[1] + g_cameraMoveRate * Math.sin(g_thirdTheta),
          0.5
        ];
      }
      break;
